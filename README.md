...
- 🕸️ Explore a clickable relationship graph with shareable rank, target-type, relationship, and evidence filters plus hover previews and selected-edge details
...
`npm run smoke`, `npm run api:contracts`, `npm run e2e`, and `npm run visual:qa` expect the app to be running locally. `npm run trace:check` expects a completed `npm run build` and asserts API route traces keep the AG2 bridge while excluding repo clutter and local bot files. `npm run e2e` clicks through the `Q42` relationship graph and asserts tab/filter URL state, and `npm run visual:qa` captures portfolio screenshots while checking for horizontal overflow. Curation export tests assert QuickStatements drafts remain comment-only until a human adds sources and approval, while source-hint tests keep reference URL, stated-in Wikidata links, retrieved-date, and formatter-aware external-ID summaries stable.
...
- `lib/search-url-state.mjs`: tested shareable tab and graph-filter URL state helpers
...
- `scripts/test-search-interaction.mjs`: browser interaction test for data-quality summary, graph filtering, URL state, traversal, and direct PID lookup
