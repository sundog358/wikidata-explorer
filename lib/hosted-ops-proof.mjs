...
async function proveWorkspaceStore(config, options = {}) {
  ...
  try {
    ...
    if (listed.response.status !== 200 || !listed.body.slots?.some((slot) => slot.id === ids.slotId)) {
      throw new Error(`workspace store list proof failed with ${listed.response.status} ${JSON.stringify(listed.body)}`);
    }

    return { ok: true, accountId: ids.accountId, projectId: ids.projectId, slotId: ids.slotId };
  } finally {
