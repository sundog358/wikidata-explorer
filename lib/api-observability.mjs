...
const API_OBSERVABILITY_RECEIVER_FILE = "api-observability-events.json";
...
export function apiObservabilityReceiverStoreConfig(env = {}) {
  const rootDir = cleanOneLine(env.API_OBSERVABILITY_STORE_DIR || "", 500);
  if (!rootDir) return { enabled: false, mode: "memory", reason: "store-dir-not-configured" };

  const resolvedRoot = path.resolve(rootDir);
  const filePath = path.resolve(resolvedRoot, API_OBSERVABILITY_RECEIVER_FILE);
  const relative = path.relative(resolvedRoot, filePath);
  if (relative.startsWith("..") || path.isAbsolute(relative)) {
    return { enabled: false, mode: "memory", reason: "invalid-store-dir" };
  }
...
function receiverStorageSummary(config) {
  return {
    mode: config?.enabled ? "filesystem" : "memory",
    durable: Boolean(config?.enabled),
  };
}

export async function readApiObservabilityReceiverStore(options = {}) {
  const config = options.config || apiObservabilityReceiverStoreConfig(options.env || process.env || {});
  if (!config.enabled) return receiverEventStore();

  try {
    const raw = await readFile(config.filePath, "utf8");
    return boundedReceiverEvents(JSON.parse(raw));
  } catch (error) {
...
export async function writeApiObservabilityReceiverStore(events = [], options = {}) {
  const config = options.config || apiObservabilityReceiverStoreConfig(options.env || process.env || {});
  const bounded = boundedReceiverEvents(events);
  if (!config.enabled) {
    const store = receiverEventStore();
    store.splice(0, store.length, ...bounded);
    return { written: true, storage: receiverStorageSummary(config) };
  }

  await mkdir(config.rootDir, { recursive: true });
  const tempPath = `${config.filePath}.${Date.now()}.tmp`;
  await writeFile(tempPath, JSON.stringify(bounded, null, 2), "utf8");
  await rename(tempPath, config.filePath);
  return { written: true, storage: receiverStorageSummary(config) };
}

export async function receiveApiObservabilityMonitorPayloadWithStore(payload = {}, options = {}) {
  const config = options.config || apiObservabilityReceiverStoreConfig(options.env || process.env || {});
  const store = options.store || await readApiObservabilityReceiverStore({ config });
  const received = receiveApiObservabilityMonitorPayload(payload, { ...options, store });
  await writeApiObservabilityReceiverStore(store, { config });
  return {
    ...received,
    storage: receiverStorageSummary(config),
  };
}

export async function apiObservabilityReceiverSnapshotWithStore(options = {}) {
  const config = options.config || apiObservabilityReceiverStoreConfig(options.env || process.env || {});
  const store = options.store || await readApiObservabilityReceiverStore({ config });
  return {
    ...apiObservabilityReceiverSnapshot({ ...options, store }),
    storage: receiverStorageSummary(config),
  };
}
