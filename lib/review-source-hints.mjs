...
function appendExternalIdToFormatter(formatter, encodedIdentifier) {
  if (/[/?#=&]$/.test(formatter)) return `${formatter}${encodedIdentifier}`;
  return `${formatter.replace(/\/+$/, "")}/${encodedIdentifier}`;
}
...
function formatterExternalIdUrl(formatterUrl, value) {
  ...
  if (!formatter || !identifier || !isHttpUrl(formatter)) return "";

  const encodedIdentifier = encodeURIComponent(identifier);
  const placeholderPattern = /\$1(?!\d)|%24(?:1|%31)|\{(?:1|id|identifier|external[-_ ]?id|value)\}/gi;
  if (placeholderPattern.test(formatter)) {
    return formatter.replace(placeholderPattern, encodedIdentifier);
  }

  return appendExternalIdToFormatter(formatter, encodedIdentifier);
}
