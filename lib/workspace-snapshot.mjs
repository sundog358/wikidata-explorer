...
function cleanRedactedOneLine(value, maxLength = 300) {
  return cleanOneLine(redactSecrets(value), maxLength);
}
...
function sanitizeWorkspaceSourceHints(hints = []) {
  if (!Array.isArray(hints)) return [];

  return hints.slice(0, 8).map((hint) => {
    if (!hint || typeof hint !== "object" || Array.isArray(hint)) return null;
    const value = cleanRedactedOneLine(hint.value, 500);
    const label = cleanRedactedOneLine(hint.label || hint.kind || "Source hint", 160);
    if (!value && !label) return null;

    const url = cleanRedactedOneLine(hint.url, 600);
    return {
      kind: cleanRedactedOneLine(hint.kind || "source", 80),
      label,
      value,
...
export function sanitizeWorkspaceCurationTasks(tasks = []) {
  if (!Array.isArray(tasks)) return [];

  return tasks.slice(0, 100).map((task) => {
    if (!task || typeof task !== "object" || Array.isArray(task)) return null;

    const id = cleanOneLine(task.id, 320);
    const entityId = normalizeEntityId(task.entityId);
    const propertyId = cleanOneLine(task.propertyId, 20).toUpperCase();
    if (!id || !entityId || !/^P\d+$/.test(propertyId)) return null;

    const status = REVIEW_STATUSES.has(task.status) ? task.status : "needs_review";
    return {
      id,
      entityId,
      propertyId,
      propertyLabel: cleanRedactedOneLine(task.propertyLabel || propertyId, 160),
      statementId: cleanRedactedOneLine(task.statementId || id, 240),
      severity: task.severity === "high" ? "high" : "medium",
      status,
      title: cleanRedactedOneLine(task.title || "Review statement evidence", 220),
      detail: cleanRedactedOneLine(task.detail, 700),
      value: cleanRedactedOneLine(task.value, 500),
      sourceHints: sanitizeWorkspaceSourceHints(task.sourceHints),
    };
  }).filter(Boolean);
}
...
export function buildWorkspaceSnapshot(input = {}) {
  ...
  return {
    ...
    review: {
      ...
      curationTasks: sanitizeWorkspaceCurationTasks(input.curationTasks),
    },
...
export function parseWorkspaceSnapshot(value) {
  ...
  return {
    ...
    snapshot: buildWorkspaceSnapshot({
      ...
      curationTasks: parsed.review?.curationTasks,
