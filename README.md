...
Set `OPENAI_API_KEY` to a valid project key. `OPENAI_MODEL` is optional and defaults to `gpt-4o-mini`. AG2 runs through `AG2_CONDA_ENV=wikidata` by default; set `AG2_PYTHON` to a Python executable if you want to bypass conda discovery.
...
- `lib/ag2.ts`: Next.js-to-AG2 process bridge
