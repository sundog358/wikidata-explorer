...
const ACCOUNT_STORE_DIR = "accounts";
...
export function normalizeWorkspaceAccountId(value = "") {
  const id = cleanOneLine(value || "", 64);
  if (!id) return "";
  return /^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$/.test(id) ? id : "";
}
...
function projectFilePath(config, projectId, accountId = "") {
  const normalizedProjectId = normalizeWorkspaceProjectId(projectId);
  if (!normalizedProjectId) return null;

  const normalizedAccountId = normalizeWorkspaceAccountId(accountId);
  if (accountId && !normalizedAccountId) return null;

  const rootDir = path.resolve(config.rootDir);
  const filePath = normalizedAccountId
    ? path.resolve(rootDir, ACCOUNT_STORE_DIR, normalizedAccountId, `${normalizedProjectId}.json`)
    : path.resolve(rootDir, `${normalizedProjectId}.json`);
...
export async function readProjectWorkspaceSlots(options = {}) {
  ...
  const accountId = normalizeWorkspaceAccountId(options.accountId);
  if (options.accountId && !accountId) return { ok: false, status: 400, reason: "invalid-account-id", accountId: "", projectId, slots: [] };

  const filePath = projectFilePath(config, projectId, accountId);
  if (!projectId || !filePath) return { ok: false, status: 400, reason: "invalid-project-id", projectId: "", slots: [] };

  try {
    const value = await readFile(filePath, "utf8");
    return { ok: true, accountId, projectId, slots: readWorkspaceSlotCollection(value) };
  } catch (error) {
    if (error?.code === "ENOENT") return { ok: true, accountId, projectId, slots: [] };
    return { ok: false, status: 500, reason: "store-read-failed", accountId, projectId, slots: [] };
  }
}
...
export async function writeProjectWorkspaceSlots(options = {}) {
  ...
  try {
    ...
    return { ok: true, accountId, projectId, slots };
  } catch {
    return { ok: false, status: 500, reason: "store-write-failed", accountId, projectId, slots: [] };
  }
}
...
export async function upsertProjectWorkspaceSlot(options = {}) {
  ...
  const current = await readProjectWorkspaceSlots({ config, accountId: options.accountId, projectId: options.projectId });
