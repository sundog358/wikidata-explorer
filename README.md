...
`npm run smoke`, `npm run e2e`, and `npm run visual:qa` expect the app to be running locally. `npm run e2e` clicks through the `Q42` relationship graph, and `npm run visual:qa` captures portfolio screenshots while checking for horizontal overflow.
