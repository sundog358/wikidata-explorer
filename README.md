...
These tracked screenshots are refreshed from the visual QA flow. `npm run visual:qa` also captures `/agents` and `/docs` desktop/mobile QA views under `.tmp/visual-qa` and fails on horizontal overflow or browser console/page errors.
...
Run `npm run verify` before shipping code changes. Run `npm run smoke`, `npm run e2e`, and `npm run visual:qa` with the local dev server running to catch route, interaction, console, hydration, and layout regressions such as `/search?q=Q42` failures.
