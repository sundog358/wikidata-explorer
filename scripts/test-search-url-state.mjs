...
const state = readSearchWorkbenchState("?q=Q42&tab=compare&gdepth=property&glayout=property&gkind=item&grank=normal&gprop=p31&gevidence=referenced&gfocus=q5");
assert.equal(state.tab, "compare");
assert.deepEqual(state.graphFilters, { depth: "property", layout: "property", kind: "item", rank: "normal", propertyId: "P31", evidence: "referenced" });
assert.equal(state.graphFocusId, "Q5");

const invalid = readSearchWorkbenchState("?tab=bad&gdepth=wide&glayout=spiral&gkind=unknown&grank=old&gprop=Q42&gevidence=nope&gfocus=bad");
...
const updated = writeSearchWorkbenchState("?q=Q42&agent=graph", {
  tab: "statements",
  graphFilters: { depth: "2", layout: "property", kind: "property", rank: "preferred", propertyId: "P279", evidence: "qualified" },
  graphFocusId: "P361",
});
...
assert.equal(updated.get("glayout"), "property");
...
const cleaned = writeSearchWorkbenchState(updated, { tab: "graph", graphFilters: { depth: "1", layout: "radial", kind: "all", rank: "all", propertyId: "all", evidence: "all" }, graphFocusId: null });
